...
void flush_dedupe_entries(struct f2fs_sb_info *sbi)
{
	...
	for(j=0;j<sbi->dedupe_info.dedupe_block_count/256;j++)
	{
		if(sbi->dedupe_info.dedupe_aa[j].dedupe_md)
		{
			for(i=j * 256; i< j*256 + sbi->dedupe_info.dedupe_aa[j].dedupe_block_count; i++)
			{
				struct page *page = NULL;
				...
				if(test_and_clear_bit(i, (long unsigned int *)sbi->dedupe_info.dedupe_md_dirty_bitmap))
				{
					...
					memcpy(page_address(page),sbi->dedupe_info.dedupe_aa[j].dedupe_md + (i - j * 256) * DEDUPE_PER_BLOCK  , DEDUPE_PER_BLOCK * sizeof(struct dedupe));
					//memcpy(page_address(page),((char *)sbi->dedupe_info.dedupe_aa[0].dedupe_md + i*(DEDUPE_PER_BLOCK * sizeof(struct dedupe))) , DEDUPE_PER_BLOCK * sizeof(struct dedupe));
					set_page_dirty(page);
					f2fs_put_page(page, 1);
				}
			}
